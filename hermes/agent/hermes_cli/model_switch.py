...
def list_authenticated_providers(
    ...
    # --- 5. Apply model_picker config filters ---
    try:
        from hermes_cli.config import load_config
        _picker_cfg = load_config().get("model_picker", {})
    except Exception:
        _picker_cfg = {}

    _picker_providers = _picker_cfg.get("providers", [])
    if _picker_providers and isinstance(_picker_providers, list):
        # Normalize allowlist to lowercase for case-insensitive matching
        _allow_set = {p.strip().lower() for p in _picker_providers if isinstance(p, str)}
        if _allow_set:
            filtered = []
            for r in results:
                slug_lower = r["slug"].lower()
                # Strip "custom:" prefix for matching (e.g. "custom:nvidia" → "nvidia")
                match_slug = slug_lower.split(":", 1)[-1] if slug_lower.startswith("custom:") else slug_lower
                if match_slug in _allow_set or slug_lower in _allow_set:
                    filtered.append(r)
            results = filtered

    _free_only = _picker_cfg.get("openrouter_free_only", False)
    if _free_only:
        for r in results:
            if r["slug"].lower() == "openrouter":
                # Filter to only free models (tagged :free or in OPENROUTER_MODELS with "free" tag)
                free_models = []
                for mid, tag in OPENROUTER_MODELS:
                    if tag == "free" or ":free" in mid:
                        free_models.append(mid)
                r["models"] = free_models[:max_models]
                r["total_models"] = len(free_models)
